...
class Sandtris:
        ...
        for i in range(self.pf.pf.shape[0]):
            for j in range(self.pf.pf.shape[1]):
                if self.pf.pf[i, j] != 0:
                    ...
                    self.canvas.create_rectangle(
                        j * self._block_size / self.scale,
                        i * self._block_size / self.scale,
                        (j + 1) * self._block_size / self.scale,
                        (i + 1) * self._block_size / self.scale,
                        fill=self._color_dict[self.pf.pf[i, j]],
                        tags=self.pf_tags[-1],
                    )
    ...
    def sand_gravity(self):
        ...
        if not same:
            ...
            self.draw_pf()
            self.root.after(
                int((300 / self.scale) / self.level), lambda: self.sand_gravity()
            )
        else:
            self.gravity_flag = False
            ...
            for i in range(self.t.block.shape[0]):
                for j in range(self.t.block.shape[1]):
                    if self.t.block[i, j] != 0:
                        ...
                        self.canvas.create_rectangle(
                            (j + self.t.c) * self._block_size / self.scale,
    ...
    def gravity(self):
        ...
        if self.check_edge() and self.check_hit():  # add or hits solid
            self.draw_tetromino()
            self.root.after(
                int((1000 / self.scale) / self.level), lambda: self.gravity()
            )
        else:
            self.t.r -= 1
